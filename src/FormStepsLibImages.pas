...
uses
	...
	System.ImageList, FMX.ImgList, KrDnceUtilTypes, System.Generics.Collections;
...
type
	...
	TStepsLibImagesForm = class(TForm)
		ToolBar1: TToolBar;
		Button2: TButton;
		ListBox1: TListBox;
		Footer: TToolBar;
...
var
	StepsLibImagesForm: TStepsLibImagesForm;
...
uses
	System.IOUtils, KrDnceGraphTypes, DModStepsLibMain;
...
procedure TStepsLibImagesForm.AddImage(const AFileName: string;
		ABitmap: TBitmap);
	var
	b1: TBitmap;
	l: TListBoxItem;
...
procedure TStepsLibImagesForm.Button2Click(Sender: TObject);
	var
	i: Integer;
	b: TBitmap;
...
procedure TStepsLibImagesForm.Button3Click(Sender: TObject);
	var
	i,
	m: Integer;
	p: TAlphaColor;
...
procedure TStepsLibImagesForm.Clear;
	var
	i: Integer;
	img: TImgImport;
...
function TStepsLibImagesForm.ColourForMap(const AMap: Integer;
		const ADefault: TAlphaColor): TAlphaColor;
	begin
	Result:= ADefault;
...
procedure TStepsLibImagesForm.ComboBox1Change(Sender: TObject);
	var
	c: TComboBox;
...
procedure TStepsLibImagesForm.DoDisplayImage(const AImage: Integer);
	var
	i: Integer;
	img: TImgImport;
...
procedure TStepsLibImagesForm.DoRebuildMapView(const AImage: Integer);
	var
	x,
	y,
	i,
...
procedure TStepsLibImagesForm.DoRemapImageColour(const AImage: Integer;
		const AColour: TAlphaColor; const AMap: Integer);
	var
	i: Integer;
	img: TImgImport;
...
procedure TStepsLibImagesForm.DoUpdateImageView(const AImage: Integer);
	begin
...
procedure TStepsLibImagesForm.FormCreate(Sender: TObject);
	begin
	FSelectedImage:= -1;
...
procedure TStepsLibImagesForm.FormDestroy(Sender: TObject);
	begin
	Clear;
	FImportImgs.Free;
	end;

function TStepsLibImagesForm.GetImageCount: Integer;
	begin
    Result:= FImportImgs.Count;
	end;

procedure TStepsLibImagesForm.ImageToScreen(const AImage: Integer;
...
procedure TStepsLibImagesForm.ListBox1Change(Sender: TObject);
	begin
...
function TStepsLibImagesForm.ShowAddImages(
