...
uses
	Windows, MMSystem, ActiveX, SysUtils, Math;
...
//	FBufferSize:= ARR_VAL_BUFFERSIZE[ABufferSize];
	...
	if  FDSound.SetCooperativeLevel(0{Application.MainFormHandle},
			DSSCL_PRIORITY) <> DS_OK then
	if  FDSound.SetCooperativeLevel(StrToInt(AParams.Values['Handle']),
			DSSCL_PRIORITY) <> DS_OK then
